...
class Functions
{
     ...
     */
    public static function secure($post, $get) //USE 1 TIMES
    {
    ...
    public static function getIP() //NOT USED
    {
     ...
     */
    public static function truncate($msg, $limit) //NOT USED
    {
     ...
     *
     * @return bool true if the reference is found
     */
    ...
    */
    ...
    public static function generateSecretkey($file)
    {
    ...
    public static function getSecretKey($file)
    {
    ...
    public static function localize($plugin_path)
    {
        ...
        if (DEBUG) {
            error_log($locale_path);
        }
        if (file_exists($locale_path)) {
            if (DEBUG) {
                echo 'Loaded translation file: '.$locale_path.'<br><br>';
            }
            require $locale_path;
        }
    }
    ...
    public static function stopProcessWithSocket($process_name, $object_name)
    {
        ...
        foreach ($processes as $process) {
            ...
            self::sendto_socket($socket, "stop\n");
            //echo "killing socket:".$socket;
        }
    }

    public static function getSocket($process_name, $object_name)
    {
        $check_process_command = 'ps aux|grep "'.$process_name.'"|grep "'.$object_name.'"'.'| grep -v grep';
        exec($check_process_command, $processes, $exitcode);
        foreach ($processes as $process) {
...
//Generate a command to start a listener
    public static function listener_generate_command($name, $objectManager, $custom_vars = false)
    {
        ...
        if (DEBUG) {
            $cmd = $name.' '.$custom_vars.' '.$listener_port.' '.self::get_yanapath().' '.DB_NAME.' '.$table.' DEBUG';
            echo 'Command:'.$cmd;
        } else {
            $cmd = $name.' '.$custom_vars.' '.$listener_port.' '.self::get_yanapath().' '.DB_NAME.' '.$table;
        }
    ...
    {
        $cmd = 'php-cgi '.self::get_yanapath().'action.php action='.$objectManager->name().'_listener_change_state';
    ...
    {
        $listener_port = $objectManager->LISTENER_PORT;
        if (self::is_port_open($listener_port)) {
            self::sendto_socket($listener_port, 'stop');
            if (DEBUG) {
    ...
    public static function listener_update($objectManager)
    {
        $listener_port = $objectManager->LISTENER_PORT;
        if (self::is_port_open($listener_port)) {
            self::sendto_socket($listener_port, 'update');
            if (DEBUG) {
    ...
    {
        $state = self::is_port_open($objectManager->LISTENER_PORT);
...
//Check Permissions of a file
    ...
    {
        $file = self::get_yanapath().$file;
        //Check if file exists
        if (file_exists($file)) {
            $perm = fileperms($file);
            if ($perm == '36333') {
                if (DEBUG) {
                    echo '<br>Permissions</br><code>'.$file.' '.$perm.'</code>';
                }
            } else {
                if (DEBUG) {
                    echo '<br>Permissions</br><code>'.$file.' '.$perm.'</code>';
    ...
    public static function time_since($seconds)
    {
        ...
        $time = self::readableTime($time_since);

        return self::longtime($time);
    }

    public static function time_since_smaller($seconds)
    {
        $time_since = time() - $seconds;
        $time = self::readableTime($time_since);

        return self::smalltime($time);
    }

    //Display readable time
    public static function readableTime($seconds)
    {
        $time['y'] = floor($seconds / 60 / 60 / 24 / 365);
        $time['d'] = floor($seconds / 60 / 60 / 24) % 365;
        $time['h'] = floor(($seconds / 3600) % 24);
        $time['m'] = floor(($seconds / 60) % 60);
        $time['s'] = $seconds % 60;

        return $time;
    }

    public static function longtime($time)
    {
        $string = '';
    ...
    public static function smalltime($time)
    {
        $string = '';
